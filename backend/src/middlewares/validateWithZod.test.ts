...
describe('validateWithZod', () => {
  ...
  describe('success:query', () => {
    app.get('/rooms', validateWithZod(validateLimitOffset, 'query'), (req: Request, res: Response) => {
      res.status(200).json({ success: true, message: 'Access granted', query: {
  ...
  describe('success:body', () => {
    app.post('/rooms', validateWithZod(createRoomSchema, 'body'), (req: Request, res: Response) => {
      res.status(200).json({ success: true, message: 'Access granted', name: req.body.name, capacity: req.body.capacity });
    });
